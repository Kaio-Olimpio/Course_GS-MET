...
Now that we loaded and visualized the adjmeanstypic data, we will load the genomic data. Here, we will also use the whole matrix:
...
```{r echo=TRUE, warning=FALSE}
BRR = BGLR(y = adjmeans$Y,ETA = ETA, nIter = 30000, burnIn = 1000, 
           thin = 5, verbose = F)
...
```{r echo=TRUE, warning=FALSE}
RKHS = BGLR(y = adjmeans$Y,ETA = ETA, nIter = 30000, burnIn = 1000, 
           thin = 5, verbose = F)
...
for(a in 1:nrept){
  for(j in 1:nfolds){
    folds <- cvFolds(nlevels(adjmeans$geno),type = "random", K = nfolds, R = 1)
...
for (z in 1:length(fold.list)) {
  for (i in 1:nfolds){
    ...
    # Fitting model 
    RKHS_M <- BGLR(y = train_data$Y, ETA = ETA, nIter = 1000, burnIn = 30, 
                 thin = 5, verbose = F)
    
    # GEBV
    Pred <- data.frame(Yhat = RKHS_M$yHat, G = adjmeans$geno)
    rownames(Pred) <- rownames(adjmeans$geno)
  ...
  # Log
  log <- all(GEBV$G == adjmeans$geno)
  
  # Results
  Out[[z]] <- data.frame(
    Rep = z,
    Log = log,
    Ac = round(cor(GEBV$Yhat, adjmeans$Y, use = "na.or.complete"), 3),
    MSPE = round(mean(((GEBV$Yhat - adjmeans$Y)^2), na.rm = T), 3),
    Slope = round(unname(lm(adjmeans$Y ~ GEBV$Yhat)$coefficients[2]),3)
  )
}
