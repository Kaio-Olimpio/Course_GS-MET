...
```{r echo=TRUE, warning=FALSE}
BRR = BGLR(y = adjmeans$Y,ETA = ETA, nIter = 15000, burnIn = 5000, 
           thin = 5, verbose = F)
...
```{r echo=TRUE, warning=FALSE}
RKHS = BGLR(y = adjmeans$Y,ETA = ETA, nIter = 15000, burnIn = 5000, 
           thin = 5, verbose = F)
...
for (z in 1:length(fold.list)) {
  for (i in 1:nfolds){
    ...
    # Fitting model 
    CV_M <- BGLR(y = train_data$Y, ETA = ETA, nIter = 15000, burnIn =5000, 
                 thin = 5, verbose = F)
