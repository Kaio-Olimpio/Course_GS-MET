...
Both phenotypic and genomic data are available online at this [Github page](https://github.com/samuelbfernandes/Trait-assisted-GS), which refers to the article of @fernandes_efficiency_2018.

We can load the phenotypic data directly from the Github using the following command:
...
```{r echo=FALSE}
head(pheno,10) %>% kbl(escape = F, align = 'c') %>% 
...
```{r echo=TRUE}
pheno_L = droplevels(subset(pheno, LOC == 13))
```
...
Now we have a square matrix whose number of rows and columns is equal to the number of genotypes. This is the G matrix:
...
The Ridge Regression BLUP, or RRBLUP, will predict the marker effect. In the RRBLUP, we will use the matrix of markers directly. For this purpose, we will use only genotypes that were both phenotyped and genotyped:

```{r echo=TRUE}
# Selecting the phenotypes with SNP information
length(levels(pheno_L$GENO))
pheno_L = droplevels(pheno_L[pheno_L$GENO %in% rownames(SNPs), ])
length(levels(pheno_L$GENO))

# Filtering the SNPs matrix: only genotyped and phenotyped
index = rownames(SNPs) %in% pheno_L$GENO
SNPs_L = SNPs[index,]
dim(SNPs_L)
```

Note that 38 genotypes were dropped from the SNPs matrix and the data containing the phenotypes. Those did not have both phenotypic and genomic information.

Now that we filtered our data, we can employ the RRBLUP. For this purpose, we will use the [rrBLUP](https://cran.r-project.org/web/packages/rrBLUP/rrBLUP.pdf) package [@endelman_2011]. In the code below, *y* is for the vector with the phenotypic data, *Z* is where we will insert the SNPs matrix, *K* is for a covariance matrix for the random effects, which will be and identity matrix by default; and *X* is a design matrix for the fixed effects, which will be a vector of ones (**1**) by default. Note that we are returning to the "1, 0, -1" codification in the SNPs matrix. This is a requirement of the _rrBlUP_ package.

```{r echo=TRUE}

RRBLUP = mixed.solve(y = pheno_L$Y, Z = SNPs_L-1, K = NULL, X = NULL)
...
After obtaining the GEBV, we can calculate the accuracy of prediction. The correlation between the GEBV and the actual phenotype represents this parameter:

```{r echo=TRUE}

acc_RRBLUP = cor(pheno_L$Y, GEBV, use = "complete.obs");acc_RRBLUP
...
```{r echo=TRUE}
GBLUP = mixed.solve(pheno_L$Y, K = A.mat(SNPs_L-1))
...
As in the RRBLUP, we may calculate the accuracy of prediction using the correlation of the phenotypes with GEBVs:

```{r echo=TRUE}

acc_GBLUP = cor(pheno_L$Y, GBLUP$u, use = 'complete.obs');acc_GBLUP
...
BRR = BGLR(y = pheno_L$Y, ETA = ETA, nIter = 3000, burnIn = 1000, 
           thin = 5, verbose = F)

BRR_GENO = data.frame("Genotype" = pheno_L$GENO,
                      "Yield" = pheno_L$Y,
                      "GEBV" = BRR$yHat)
...
BayesB = BGLR(y = pheno_L$Y, ETA = ETA, nIter = 3000, burnIn = 1000, 
           thin = 5, verbose = F)
...
```{r echo=TRUE}
pheno_L = pheno_L[order(pheno_L$GENO, decreasing = FALSE),] 
SNPs_L = SNPs_L[order(row.names(SNPs_L)),]
all(rownames(SNPs_L) == pheno_L$GENO)
```
...
```{r echo=TRUE}
pheno_L$ID = factor(1:nrow(pheno_L))
```
...
for(a in 1:nrept){
  for(j in 1:nfolds){
    folds <- cvFolds(nlevels(pheno_L$ID),type = "random", K = 5, R = 1)
...
for (z in 1:length(fold.list)) {
  for (i in 1:nfolds){
    ...
    # Fitting model 
    BB_M <- BGLR(y = train_data$Y, ETA = ETA, nIter = 3000, burnIn = 1000, 
                 thin = 5, verbose = F)
    
    # GEBV
    Pred <- data.frame(Yhat = BB_M$yHat, G = pheno_L$ID)
  ...
  # Log
  log <- all(GEBV$G == pheno_L$ID)
  
  # Results
  Out[[z]] <- data.frame(
    Rep = z,
    Log = log,
    Ac = round(cor(GEBV$Yhat, pheno_L$Y, use = "na.or.complete"), 3),
    MSPE = round(mean(((GEBV$Yhat - pheno_L$Y)^2), na.rm = T), 3)
  )
}
...
The object "Out" is divided by repetition. In the "Out" objects for each repetition, "Rep" is the number of the repetition, "Log" is a diagnostic indicating if the order of the predicted breeding values matches the order of the phenotypic data, "Ac" is the accuracy of prediction (correlation between the GEBV and phenotypes), and "MSPE" is the mean square prediction error (the lower, the better).
